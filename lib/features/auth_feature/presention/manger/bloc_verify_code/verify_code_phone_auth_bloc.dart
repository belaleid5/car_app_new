...
import 'package:car_app_new/features/auth_feature/data/models/request/verify_code_phone_model_request.dart';
...
class VerifyCodePhoneAuthBloc
    extends Bloc<VerifyCodePhoneAuthEvent, VerifyCodePhoneAuthState> {
  VerifyCodePhoneAuthBloc(this._repo)
      : super(const VerifyCodePhoneAuthState.initial()) {
  ...
  Future<void> _onEvent(
    ...
    await event.when(
      verifyPhone: (phoneNumber, accessToken) async {
        ...
        result.when(
          success: (VerifyPhoneResponseModel response) {
            emit(
              VerifyCodePhoneAuthState.success(response: response),
            );
          },
          failure: (error) {
            emit(
              VerifyCodePhoneAuthState.failure(error: error),
            );
          },
        );
      },
